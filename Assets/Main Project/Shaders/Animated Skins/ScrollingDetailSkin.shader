Shader "Frog-go/Animated Skins/Scrolling Additive" {
	Properties {
		_Color ("Main Color", Color) = (1,1,1,1)
		_MainTex ("Base (RGB)", 2D) = "white" {}
		_DetailTex ("Additive Detail (RGB)", 2D) = "black" {}
		_MaskTex ("Detail Mask (R)", 2D) = "white" {}
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutput o) {
			...
			fixed4 add = tex2D(_DetailTex, scrollUv) * mask;

			o.Albedo = c.rgb + add.rgb;
			o.Alpha = c.a;
		}
		ENDCG
	}
